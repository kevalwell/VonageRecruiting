...
public class parseInt {
	...
	private int parse(String text) 
	{
		...
		//Handling case where text is empty or null
		...
		//Initial DFA state is BEGIN
		stateDFA state = stateDFA.BEGIN;
		
		//Loop along entire length of the string
		for (int i = 0; i < length; i++)
		{
			char c = text.charAt(i);
			
			//If current character is a numerical digit, update
			//number with correct radix (decimal in our case)
			if (Character.isDigit(c))
			{
				number *= RADIX;
				number += Character.digit(c, RADIX);
			}
			switch (state)
			{
			case BEGIN:
